...
def positional_encoding_2d(coordinates, C):
    ...
    freqs = torch.pow(10000,
                      torch.arange(0, d, dtype=torch.float32) /
                      d).unsqueeze(0).unsqueeze(0).to(coordinates.device)
...
def depth_to_3d(depth_image, intrinsics):
    ...
    pixels_homogeneous = np.stack((x + 0.5, y + 0.5, ones),
                                  axis=-1).reshape(-1, 3)
...
def create_occupancy_grid(points_3d,
                          grid_size,
                          grid_points,
                          use_occ_only=False):
    occupancy_grid = torch.ones(
        (grid_points[0], grid_points[1], grid_points[2])).float() * -1
    ...
    indices = (points_3d[:, :3] -
               grid_lower_boud.unsqueeze(0)) // voxel_size.unsqueeze(0)
...
def create_ground_map(points_3d, grid_size, grid_points):
    ground = torch.zeros((grid_points[0], grid_points[2])).float()
    voxel_size_x = (grid_size[1] - grid_size[0]) / grid_points[0]
    voxel_size_y = (grid_size[3] - grid_size[2]) / grid_points[1]
    voxel_size_z = (grid_size[5] - grid_size[4]) / grid_points[2]
    voxel_size = torch.Tensor([voxel_size_x, voxel_size_y, voxel_size_z])
    ...
    # Add ground height context
    ground_points = points_3d[points_3d[:, 3] == 0.]
    ground_indices = (ground_points[:, :3] -
                      grid_lower_bound.unsqueeze(0)) // voxel_size.unsqueeze(0)
    ground_indices = ground_indices.long()
    ground[ground_indices[:, 0], ground_indices[:, 2]] = ground_points[:, 1]
...
def rotation_matrix_to_angle_axis(rotation_matrix):
    ...
    if rotation_matrix.size(-1) == 3:
        rotation_matrix = convert_rotation_matrix_to_homogeneous(
            rotation_matrix)
